...
describe("Tests of bookSelection object:", function () {
  describe("Tests of isGenreSuitable function:", function () {
    it("tests the value of string is Horror or Thriller, and the value of age is less or equal to 12", function () {
      ...
      let age1 = 12;
      let age2 = 22;
      expect(bookSelection.isGenreSuitable(genre1, age1)).to.equal(
        `Books with ${genre1} genre are not suitable for kids at ${age1} age`
      );
      expect(bookSelection.isGenreSuitable(genre2, age1)).to.equal(
        `Books with ${genre2} genre are not suitable for kids at ${age1} age`
      );
      expect(bookSelection.isGenreSuitable(genre1, age2)).to.equal(
        `Those books are suitable`
      );
      expect(bookSelection.isGenreSuitable(genre2, age2)).to.equal(
        `Those books are suitable`
      );
    });
  ...
  describe("Tests of isItAffordable function:", function () {
    it("check if book is affordable", function () {
      ...
      expect(bookSelection.isItAffordable(price1, budget)).to.equal(
        `Book bought. You have ${result}$ left`
      );
      expect(bookSelection.isItAffordable(price2, budget)).to.equal(
        "You don't have enough money"
      );
    });
    it("checking if input is numbers", function () {
      expect(() => bookSelection.isItAffordable("price", "budget")).to.throw(
        "Invalid input"
      );
      expect(() => bookSelection.isItAffordable("price", 200)).to.throw(
        "Invalid input"
      );
      expect(() => bookSelection.isItAffordable(100, "200")).to.throw(
        "Invalid input"
      );
      expect(() => bookSelection.isItAffordable(undefined, 200)).to.throw(
        "Invalid input"
      );
      expect(() => bookSelection.isItAffordable(100, undefined)).to.throw(
        "Invalid input"
      );
    });
  });
  ...
  describe("Tests of suitableTitles function:", function () {
    it("if books genre is equal to wantedGenre", function () {
      ...
      expect(bookSelection.suitableTitles(array, "Fantasy")).to.deep.equal([
        "The Lord of the Rings",
      ]);
      expect(bookSelection.suitableTitles(array, "Fiction")).to.deep.equal([
        "The Great Gatsby",
      ]);
    ...
    it("if books is not an array and genre is not a string", function () {
      let books = 12;
      let genre = 123;
      expect(() => bookSelection.suitableTitles(books, genre)).to.throw(
        "Invalid input"
      );
    });
  });
